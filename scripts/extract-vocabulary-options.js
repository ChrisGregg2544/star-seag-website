/**
 * extract-vocabulary-options.js
 * Extracts A/B/C/D option texts from a Catapult PDF for vocabulary MC questions.
 * Filters to single-letter correct_answer only (excludes written answers).
 ...
 *
 * Usage: node scripts/extract-vocabulary-options.js "Warm Up 1 (2026).pdf" P6 1
 */
...
if (!paperFile || !yearGroup || !paperNumber) {
  console.error('Usage: node scripts/extract-vocabulary-options.js "Warm Up 1 (2026).pdf" P6 1');
  process.exit(1);
}
...
async function fetchPendingQuestions() {
  ...
  // Exclude written-answer questions (correct_answer is a full word/phrase, not a single letter)
  return rows.filter(q => q.correct_answer?.length === 1);
}
...
The PDF is the question paper. It contains vocabulary multiple-choice questions (word meaning, synonyms, definitions). Below is a list of vocabulary questions with IDs. For each question that appears in this PDF, extract its four multiple-choice options (A, B, C, D) exactly as printed.
...
[
  { "id": "uuid", "options": { "A": "...", "B": "...", "C": "...", "D": "..." } }
]

Rules:
- Copy option text exactly as printed in the paper.
...
async function main() {
  ...
  console.log('Fetching pending vocabulary MC questions from Supabase...');
  const questions = await fetchPendingQuestions();
  console.log(`Found ${questions.length} vocabulary MC questions with no options for ${yearGroup}\n`);
  ...
  for (const q of questions) {
    ...
    const missing = ['A', 'B', 'C', 'D'].filter(k => !(k in options));
    if (missing.length) {
