/**
 * extract-comprehension-options.js
 * Extracts A/B/C/D/E option texts from a Catapult PDF for comprehension_mc questions.
 ...
 *
 * Usage: node scripts/extract-comprehension-options.js "Warm Up 1 (2026).pdf" P6 1
 */
...
if (!paperFile || !yearGroup || !paperNumber) {
  console.error('Usage: node scripts/extract-comprehension-options.js "Warm Up 1 (2026).pdf" P6 1');
  process.exit(1);
}
...
The PDF is the question paper. It contains a reading passage followed by comprehension questions (typically Q16–Q22). Below is a list of comprehension multiple-choice questions with IDs. For each question that appears in this PDF, extract its five multiple-choice options (A, B, C, D, E) exactly as printed.
...
- Each question has exactly five options: A, B, C, D, E.
...
async function main() {
  ...
  console.log('Fetching pending comprehension_mc questions from Supabase...');
  const questions = await fetchPendingQuestions();
  console.log(`Found ${questions.length} comprehension_mc questions with no options for ${yearGroup}\n`);
