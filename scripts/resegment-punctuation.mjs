 ...
 *
 * E2: re-segment the quarantined punctuation AND spelling questions that have
 * overlapping / duplicated segments. Each intended sentence is reconstructed
 * and re-cut into 4 clean, non-overlapping consecutive segments A/B/C/D,
 * preserving the single mistake (punctuation OR spelling) in the correct-answer
 * segment. An independent verifier pass then approves each result.
 *   verified   → validated=true, validator_reason=null
 *   unverified → left quarantined (validated=false, validator_reason unchanged)
 *
 * Usage:
...
// Quarantined punctuation + spelling questions (segment-style, overlapping).
async function fetchBroken() {
  let all = [], from = 0, size = 1000;
  while (true) {
    const { data, error } = await sb.from('questions')
      .select('id,topic,year_group,difficulty,question_text,options,correct_answer,explanation')
      .in('topic', ['punctuation', 'spelling'])
      .eq('validated', false)
...
function buildPrompt(q) {
  ...
  const isSpelling = q.topic === 'spelling';
  const kind = isSpelling ? 'spelling' : 'punctuation';
  const errKind = isSpelling ? 'a spelling mistake (exactly one misspelled word)' : 'a punctuation/capitalisation mistake';
  const errExample = isSpelling
    ? 'one word spelled wrongly (e.g. "definately", "recieve", "seperate")'
    : 'a missing apostrophe, a lowercase proper noun, or a missing/misplaced comma';
  const cleanDesc = isSpelling ? 'every word correctly spelled' : 'perfectly punctuated';

  const ansDesc = ans === 'N'
    ? `The intended answer is N (No mistake) — the reconstructed sentence must be fully correct with ${cleanDesc} and NO errors.`
    : `The intended answer is ${ans} — segment ${ans} must contain exactly ONE ${errKind}, and the other three segments must be correct.`;

  return `A SEAG "find the ${kind} mistake" question has broken segments — they overlap or duplicate each other. Reconstruct it cleanly.
...
${ans === 'N'
  ? `4. The sentence must have ${cleanDesc} and NO errors of any kind (answer N).`
  : `4. Introduce EXACTLY ONE ${errKind}, and it must be in segment ${ans} ONLY (e.g. ${errExample}). Every OTHER segment must be flawless — do NOT introduce any second error anywhere${isSpelling ? '' : ', do not capitalise common nouns, do not add stray commas'}.`}
...
function buildVerifyPrompt(parsed, topic) {
  const o = parsed.options || {};
  const ans = String(parsed.correct_answer).toUpperCase();
  const segs = ['A', 'B', 'C', 'D'].map(k => `${k}: ${JSON.stringify(o[k] || '')}`).join('\n');
  const isSpelling = topic === 'spelling';
  const kind = isSpelling ? 'spelling' : 'punctuation';
  const cleanCheck = isSpelling
    ? '- Every word in the sentence is correctly spelled.'
    : '- The sentence is PERFECTLY punctuated with zero errors of any kind.';
  const errCheck = isSpelling
    ? `- Segment ${ans} contains exactly ONE misspelled word.\n- EVERY other segment (not ${ans}) is correctly spelled.`
  ...
  return `You are a strict SEAG examiner checking a "find the ${kind} mistake" question. Read the 4 segments as one sentence.
...
async function verify(parsed, topic) {
  const resp = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: { 'x-api-key': ANTHROPIC_KEY, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' },
    body: JSON.stringify({ model: MODEL, max_tokens: 300, system: SYSTEM, messages: [{ role: 'user', content: buildVerifyPrompt(parsed, topic) }] }),
  });
...
async function main() {
  ...
  const nP = broken.filter(q => q.topic === 'punctuation').length;
  const nS = broken.filter(q => q.topic === 'spelling').length;
  console.log(`Quarantined segment questions: ${broken.length} (punctuation ${nP}, spelling ${nS})\n`);
  const batch = broken.slice(0, LIMIT === Infinity ? broken.length : LIMIT);

  let applied = 0, stillQuarantined = 0, fail = 0, verifyPass = 0, verifyFail = 0;
  for (let i = 0; i < batch.length; i++) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3 && !parsed; attempt++) {
        ...
        let v;
        try { v = await verify(cand, q.topic); } catch { v = { valid: false, reason: 'verify error' }; }
        if (v.valid) { parsed = cand; verdict = v; }
        else verdict = v;
      }

      // Re-lint the reconstruction against the contract before trusting it
      if (parsed) {
        const newRow = {
          topic: q.topic, year_group: q.year_group, difficulty: q.difficulty,
          question_type: 'Multiple_Choice', question_text: q.question_text,
          options: parsed.options, correct_answer: String(parsed.correct_answer).toUpperCase(),
        };
        const violations = lintQuestion(newRow);
        if (violations.length > 0) { verdict = { valid: false, reason: 'contract: ' + violations.join(', ') }; parsed = null; }
      }
      ...
      if (parsed) {
        ...
        // No clean reconstruction — leave it quarantined (already validated=false)
        verifyFail++;
        stillQuarantined++;
        console.log(`UNVERIFIED → left quarantined (${verdict?.reason || 'no clean reconstruction'})`);
      }
    } catch (e) {
  ...
  if (APPLY) console.log(`  Revalidated: ${applied}   Left quarantined: ${stillQuarantined}`);
