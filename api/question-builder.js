...
function categoryGuidance(category) {
  const guides = {
    ...
    statistics:             `Generate questions about bar charts, pictograms, line graphs, pie charts, mean, and range. Describe any required diagram in diagram_description. Multiple choice A–E. CRITICAL — CLEAN NUMBERS: Mean questions MUST use values whose sum divides EXACTLY by the count, so the mean is a terminating value (a whole number, or a clean decimal like 12.5 — never a rounded/recurring result such as 33.83 or 16.4). Never key a rounded mean as the answer. Pie-chart and pictogram questions MUST be built so every sector/symbol yields a WHOLE-NUMBER count of people or objects (choose the total and the angles/symbol-values so each category divides evenly — e.g. a 90° sector of 200 people = 50, not a 120° sector of 200 = 66.67). The same principle applies ANYWHERE a question resolves to a count of discrete things (people, objects, votes, cars): the answer must be a whole number. If a computation would give a non-terminating decimal or a fractional count, change the numbers.`,
    algebra_sequences:      `Generate questions about number sequences, function machines, simple algebra (find the missing number), and patterns. Multiple choice A–E. NEVER generate two-step equations in the format ax + b = c or ax - b = c where both multiplication and addition/subtraction are required to solve. Only generate one-step equations like a + 12 = 27 or 417 ÷ f = 139 where a single operation finds the answer. The format 2p + 5 = 3p - 2 (variables on both sides) is also forbidden. CRITICAL SEAG SCOPE: Sequences must ONLY ask pupils to predict the next number, a missing number, or continue a pattern by extending it. NEVER ask for the "nth term", "term position", or any general term formula. NEVER use algebraic term-notation such as n², 2n, 3n+1, or "the nth term is given by…". Quadratic sequences and position-to-term rules are GCSE-level and out of spec. Sequences are solved by spotting the step/pattern and continuing it — never by an algebraic expression in n. Stick to the style of the real SEAG reference questions.`,
  };
...
const SPECIALIST_CATEGORIES = new Set(['punctuation', 'spelling', 'grammar', 'vocabulary', 'geometry', 'fractions_decimals', 'measurement']);
