...
- **5 broken arithmetic** → 4 function machines whose reverse-division has no integer solution (keyed answer is arithmetically wrong) `validator_reason='wrong-answer'`; 1 "84 seats ÷ 9 equally divided" `validator_reason='non-terminating-answer'`.
...
Net effect: bank 13,314 → 12,942 validated, 0 lint violations. Statistics 1,118 → 1,060. Geometry 967 → 828.
...
**Prevention follow-ups (Sept 2026):**
1. `api/question-builder.js` statistics generation guidance now requires: mean values whose sum divides EXACTLY by the count (terminating mean, never a rounded/recurring result), and pie-chart/pictogram totals+angles/symbols chosen so every category yields a WHOLE-NUMBER count — applied anywhere a question resolves to a count of discrete things.
2. `scripts/question-contract.mjs` gained two deterministic semantic checks (enforced by the gate on every insert): `non-terminating-mean` / `rounded-non-terminating-answer` (a keyed answer resting on a `÷` that doesn't resolve; excludes "to N dp" rounding questions and comparison distractors) and `non-integer-discrete-count` (a "how many <people/objects>" answer that is fractional; excludes £/%/measurement units). `lint-questions.mjs` now fetches `explanation` so the mean check can see the working.
   - **Coverage limit (by design) — the gate and the audit are NOT redundant:** the contract gate re-catches **86% (51/59)** of this fault class. The remaining **14%** are faults hidden behind rounding to a whole number (e.g. a pie chart keyed "67 people", a mean keyed "34") — structurally perfect rows the deterministic gate cannot see, because confirming them requires *reconstructing the computation* (recomputing the true mean or the sector-angle count and comparing), which a no-AI structural check cannot do. Only the `audit-scope-ai.js` Sonnet pass catches that 14%. So the periodic audit covers a class the gate structurally cannot, and is not made redundant by it. **Re-run `audit-scope-ai.js` after any significant generation batch** (and before launch), then owner-review the flags.
...
## KNOWN LATENT ISSUES (not currently biting — read before touching the relevant code)
- **study.html difficulty-fallback blind spot.** `fetchBatch()` builds the question pool with a hardcoded `diffTiers = [1,2,3,4,5]` (all difficulties), so difficulty does not currently narrow the pool. The seen-history fallback (`if (filtered.length < questionsNeeded && seenIds.length > 0)`) re-queries *without* the `seenIds` exclusion — **but it re-runs `makeTopicQuery()`, which still applies the same `diffTiers` filter.** So the fallback rescues *seen-history* depletion but NOT *difficulty* depletion. This is harmless today because all tiers are always requested, but if difficulty progression is ever re-enabled (the "difficulty unlocks after 5 correct" UI copy and the unused `correctCount` variable suggest it was the original intent), a topic stocked entirely outside the requested tier — e.g. a child capped at difficulty 1–2 practising a topic whose questions are all 3–5 — would return an **empty pool for a legitimately stocked topic**, and the fallback would not rescue it. **Any reintroduction of difficulty gating MUST also widen the tiers (fall back to all difficulties) before giving up.** The empty-pool guard (Sept 2026) means this would surface as a loud `[EMPTY-POOL]` Vercel log rather than a silent 0/0 session, but it would still block the child from practising.
